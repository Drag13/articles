...
> А вот Git появился сравнительно недавно - 2005 году благодаря [Linus Torvald](https://en.wikipedia.org/wiki/Linus_Torvalds) ключевому создателю Linux.
...
* Language - csharp. Хотя писать мы будем под F# Travis не делает различий для этих языков и директивы fsharp нет. Зато этот файл прекрасно подходит и для проектов на C#.
...
И протестируем ее, а именно то, что результат вызова этой функции будет всегда отличается от предыдущего. Что, кстати, не гарантировано, т.к. существует не нулевая вероятность того, что при генерации двух GUID нам могут нам могут попасться два одинаковых. Правда, скорее всего, пока вы дождетесь этого события потухнет солнце, но все же шанс есть.
...
Здесь все очевидно - мы запускаем тесты без билда (т.к. билд артефакты уже уже получены на предыдущем шаге) чем економим себе время.
...
Итак, тесты у нас есть, они запускаются и даже видно какой-то отчет о покрытии кода. Но и этого очевидно недостаточно. Теперь нам нужен кто-то еще, кто сможет обработать этот отчет. Здесь нам пригодится [Codecov](https://codecov.io) который, тоже бесплатен для OpenSource проектов. Логинимся в Codecov с помощью GitHub account и добавляем нужный проект. Наконец, в последний раз обновляем .travis.yml - добавляем новую секцию after_script.
