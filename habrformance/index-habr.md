![](https://habrastorage.org/webt/dg/5m/ab/dg5mabkefi6xhfiovvydhay2fkw.jpeg)
...
![](https://habrastorage.org/webt/ni/fs/yb/nifsybxewz18qswerh5jubfsn1a.png)
...
![](https://habrastorage.org/webt/0m/8g/u4/0m8gu4f9s-odx2kf7xg8jiweuke.png)
...
![](https://habrastorage.org/webt/r-/om/kq/r-omkqj5qrslwffj42o5eogv_3s.png)
...
![стильненько](https://habrastorage.org/webt/nm/6y/a7/nm6ya7qcp1bdqcp5sl2fiingrp8.png)
...
Все тоже достаточно хорошо, FCP/FMP - 1689, полезная нагрузка 45%. Кстати, подгонять загразку до 100% тоже плохо, так как более слабые машины будут перегружены. Так что лучше иметь запас на простое. Но, тут неожиданно много времени ушло на Rendering - 400 ms для FMP. Из них 200 ms ушло на пересчет стилей и перерасчет лейаута. 
...
Тут все понятно. Изображения пережать и перекодировать (ха), DOM - найти кто генерит так много node (ха-ха), выставить кеширование на статические ресурсы (ну это можно), и оторвать document.write-у его writer (три ха-ха если это чужой скрипт)
...
![ренедерим на чайнике](https://habrastorage.org/webt/td/ei/3j/tdei3jxtwbcvda5csqvvmgz2tua.png)

Как видим все стало критично хуже:
...
* Впервые появилась проблема слишком загруженного main thread. Мы порезали вычислительные мощности и браузер просто захлебнулся в javascript - (8.5 секунд для script evaluation из них 2186 ms ушло на raven.js). Не слабо да?
...
![](https://habrastorage.org/webt/qz/hb/gn/qzhbgnvpgll4xbdlvejmvfzpndm.png)
(показан частично)
...
* Сайт быстро рендериться на хорошем соединение и мощной машине, но все довольно быстро ухудшается с уменьшением канала и падением производительности процессора
* При этом, очевидно, что проведена хорошая работа по оптимизации. Сайт достаточно быстро грузит основной текст, остальное происходит уже после того, как мы видим контент. Конечно ребята молодцы, вытянули.

Итого: Первая загрузка достаточно хорошо оптимизирована (это мы, кстати, еще service-workerы не смотрели). Повторная загрузка (вспомните суперотчёт LH когда мы не отключали кэш) еще лучше. Однако с падением вычислительных мощностей или пропускной способности канала все довольно быстро становится хуже. Так что есть еще куда расти и улучшаться. И поменьше бы трекеров/аналитики/рекламы, но это уже скорее мечты мало сопоставимые с требованиями бизнеса.
