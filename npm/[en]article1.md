# How to setup npm project for you and your team with automated formatting, linting, testing and autogenerated documentation
...
Create new folder, initialize new repository and project and go to the next step.
...
Let's start from code formatting - indention types, size and so on. First tool is [.editorconfig](https://editorconfig.org/) file. It is recognized with most of IDE and helps to keep your autoformatting consistent across different IDEs and users.
...
But sometimes autoformatting can be not used. To ensure that all is well formatted [prettier](https://github.com/prettier/prettier) appears. If you forget format the code, prettier will do it for you.
...
Last command will create tslint.json for you. It already extends tslint:recommended set of rules, but you can extend or override them what ever you want. Don't forget to add lint command to your package.json.
...
Now it is time to setup our tests. Install karma and other related dependencies
...
This will setup codecoverage file and treshold level. Both are important. First one helps you to deal with your coverage and the second one keeps your coverage on a certain level.
...
And try to run it. Don't forget to write some code inside src folder and tests inside tests folder.

## Commit Style

Usually all write commits in some "random" format. So, to keep commits good enough, [commitizen](https://github.com/commitizen/cz-cli) was invented. This tool prompt few questions and generate commit for you. Another good point that we can generate changelog file from commits written with help of commitizen.
...
And update package.json with new command
...
Build is quite simple and it is just a matter to add build and clean commands to the package.json
...
Ok, the most part already finished. We created bunch of different scripts to keep our code clean and correct. But runing them each time manually is a quite booring task and can lead to mistakes. So, we need to automate them. As you know, when you make commit few git events rise - pre-commit, post-commit and so on. We can use them to run our rules before code will be commited or pushed. But there is a problem -  git hooks are not shareable. And that's why [husky](https://www.npmjs.com/package/husky) appears. This package wraps the git events and run rise your scripts from package.json. If script fails, commit will be canceled and you will get message what's going wrong.
...
Now, when you try to make a commit prettier will run and fix all formatting issues. When you try to make a push - codestyle and test will be checked automatically. You can extend this commands what ever you need like sending notifications, extra check etc.

## Publish

Great, we are almmost done. So, let's say we are ready to publish package to npm. As you know, much work should be done before - tests, documentation update, version and tags update. Easy to forget something, yeah? So, it is good idea to automate the process too. For that purposes we will use native npm hooks - preversion, version and postversion. Add next lines to scripts section your package.json
...
That's all, your awesome project is ready to go! Thanks for reading.
